<?php declare(strict_types=1);

namespace EasyAdminTest\Job;

use EasyAdmin\Job\Backup;
use EasyAdminTest\EasyAdminTestTrait;
use Omeka\Test\AbstractHttpControllerTestCase;

/**
 * Tests for EasyAdmin Backup Job.
 */
class BackupTest extends AbstractHttpControllerTestCase
{
    use EasyAdminTestTrait;

    public function setUp(): void
    {
        parent::setUp();
        $this->loginAdmin();
    }

    public function tearDown(): void
    {
        $this->logout();
        parent::tearDown();
    }

    /**
     * Test job class exists.
     */
    public function testJobClassExists(): void
    {
        $this->assertTrue(class_exists(Backup::class));
    }

    /**
     * Test job extends AbstractJob.
     */
    public function testJobExtendsAbstractJob(): void
    {
        $reflection = new \ReflectionClass(Backup::class);
        $this->assertTrue($reflection->isSubclassOf(\Omeka\Job\AbstractJob::class));
    }

    /**
     * Test job has perform method.
     */
    public function testJobHasPerformMethod(): void
    {
        $this->assertTrue(method_exists(Backup::class, 'perform'));
    }

    /**
     * Test job uses ZipTrait.
     */
    public function testJobUsesZipTrait(): void
    {
        $traits = class_uses(Backup::class);
        $this->assertContains(\EasyAdmin\Job\ZipTrait::class, $traits);
    }
}
